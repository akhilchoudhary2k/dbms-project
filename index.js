...
app.get('/del_phone/:id',(req,res) => {
    if(req.session.loggedin){
        if(req.session.user_type=="student")
        {
            console.log(req.session.username + " is trying to delete a phone.");
            var sql='select count(*) as ph_ct from student_phone where phone="'+req.params.id+'" and student_id="'+req.session.username+'"';
            connection.query(sql,(err,result)=>
            {
                if(err)
                {
                ...
                {
                    console.log(result);
                    sql='delete from student_phone where student_id="'+req.session.username+'" and phone="'+req.params.id+'"';
                    connection.query(sql,(err,result)=>
                    {
                        if(err)
                        {
                    ...
                    connection.query(sql,(err,result)=>
                        ...
                        else
                        {
                            ...
                            res.render("student_add_phone",{
                                username : req.session.username,
                                phone_list : result,
                                message : "You dont have such a phone number."
                            })
                        }
                    });
                }
        ...
        {
            console.log(req.session.username + " is trying to delete a phone.");
            var sql='select count(*) as ph_ct from professor_phone where phone="'+req.params.id+'" and prof_id="'+req.session.username+'"';
            connection.query(sql,(err,result)=>
            {
                if(err)
                {
                ...
                {
                    console.log(result);
                    sql='delete from professor_phone where prof_id="'+req.session.username+'" and phone="'+req.params.id+'"';
                    connection.query(sql,(err,result)=>
                    {
                        if(err)
                        {
...
app.get('/add_tag_proj',(req,res) => {
    if(req.session.loggedin){
	    ...
	    connection.query(sql,(err,result)=>
		{
            ...
            {
                console.log(result);
                var q = 'select * from professor,project where professor.prof_id = project.prof_id and professor.prof_id ="' + req.session.username + '"'
...
app.get('/admin_add_tag',function(req,res){
    if(req.session.loggedin){
        console.log(req.session.username + "is accessing /addproject page.");
...
app.get('/all_allocation', (req,res)=>{
    if(req.session.loggedin){
        var q = 'select student.student_id as a, student.first_name as b, project.project_id as c, project.title as d, professor.first_name as e from student , professor, project where professor.prof_id=project.prof_id and student.allocated_proj_id = project.project_id'
        connection.query( q, (err, result)=>{
            if(err){
                console.log(err)
                throw err
...
app.get('/del_project/:id',(req,res) => {
    if(req.session.loggedin){
        ...
        connection.query(sql,(err,result)=>
		{
            ...
            {
                sql='select count(*) is_allocated_proj from student where allocated_proj_id='+req.params.id;
                connection.query(sql,(err,result)=>
                {
                    if(err)
                    {
                    ...
                    else if(result[0].is_allocated_proj)
                    {
                        console.log("Prof. "+req.session.username + " is viewing projects.");
                        sql='select * from project where prof_id="'+req.session.username +'"';
                        connection.query(sql,(err,result)=>
                        {
                            ...
                            res.render("show_projects",{
                                username : req.session.username,
                                project_list : result,
                                message : "This project has been already allocated."
                            })
                        });
                    }
                    else
                    {
                        ...
                        sql='delete from project_tag where project_id='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_1=NULL where pref_1='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_2=NULL where pref_2='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_3=NULL where pref_3='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_4=NULL where pref_4='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_5=NULL where pref_5='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_6=NULL where pref_6='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_7=NULL where pref_7='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
                        ...
                        sql='update student set pref_8=NULL where pref_8='+req.params.id;
                        connection.query(sql,(err,result)=>
                        {
                            if(err)
                            {
...
app.get('/show_ta_student',(req,res)=>{
    if(req.session.loggedin){
        console.log(req.session.username + " is seeing a show_ta_student.");
            var q = 'select prof_id from professor';
            connection.query(q , (err, result) => {
                if(err){
                    console.log(err)
                    throw err
...
app.post('/admin_add_tag',(req,res)=>{
    var sql = 'select count(*) as cnt from tag where tag_name = "' + req.body.tag_name + '"';
    connection.query(sql,(err,result)=>
	{
        if(err)
        {
            ...
            console.log('kyon aakhir kyon');
            res.render("admin_add_tag",{
                message : 'Tag already exists!!.'
            })
        }
        else{
            sql='insert into tag values("' + req.body.tag_name + '","' + req.body.tag_desc +'")';
            //console.log(sql);
            console.log(req.session.username + " added a Tag.");
            connection.query(sql,(err,result)=>
            {
                if(err)
                {
...
app.get('/allocated_students',(req,res) => {
    if(req.session.loggedin)
    {
        var sql='select student.first_name,student.student_id,student.email,student.allocated_proj_id from student,project where student.allocated_proj_id=project.project_id and project.prof_id="'+req.session.username+'"';
        connection.query(sql, (err,result) => {
            if(err)
            {
                console.log(err)
...
app.get('/allocated_project',(req,res) => {
    if(req.session.loggedin)
    {
        console.log('In allocated project')
        var sql = 'select allocated_proj_id from student where student_id = "' + req.session.username + '"';
        console.log('Inside allocated project')
        connection.query(sql, (err,result) => {
            if(err)
            {
                console.log(err)
            ...
            {
                console.log(result);
                res.redirect("/view_proj_detail/"+result[0].allocated_proj_id);
            }
        })
    }
    else {
